...
namespace CLASSICDIY
{
   ...
   void IOT::Run()
   {
      ...
      { // Network not setup?, see if flasher is trying to send us the SSID/Pw
         ...
         if ((now - _FlasherIPConfigStart) > FLASHER_TIMEOUT)
         { // wait for flasher tool to send Wifi info
            logd("Done waiting for flasher!");
            setState(ApState); // switch to AP mode for AP_TIMEOUT
         }
      }
      else if (_networkState == Boot)
      {                     // have network selection, start with wifiAP for AP_TIMEOUT then STA mode
         setState(ApState); // switch to AP mode for AP_TIMEOUT
      }
      else if (_networkState == ApState)
      {
         if (_NetworkSelection != APMode)
         { // don't try to connect if in APMode
            if (_AP_Connected == false)
            { // if AP client is connected, stay in AP mode
               if ((now - _waitInAPTimeStamp) > AP_TIMEOUT)
               { // switch to selected network after waiting in APMode for AP_TIMEOUT duration
                  if (_SSID.length() > 0)
                  { // is it setup yet?
      ...
      else if (_networkState == OffLine && _NetworkSelection > APMode)
      { // went offline, try again...
      ...
      { // GPIO0 pressed for GPIO0_FactoryResetCountdown? initiate a factory reset
